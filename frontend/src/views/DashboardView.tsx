...
/** The review dashboard: batch strip, product list, field review, source panel. */
export function DashboardView() {
  const [selectedProductId, setSelectedProductId] = useState<string | null>(null)
  const [selectedField, setSelectedField] = useState<ReviewField | null>(null)
  /** The field whose contradiction detail is open, if any. */
  const [inspectedField, setInspectedField] = useState<ReviewField | null>(null)
  ...
  const sourceTable: SourceTable | null = useMemo(() => {
    ...
    return {
      ...
      // Same `key` scheme toReviewFields uses, so a row matches exactly one
      // field even when two rows share a name.
      rows: product.fields.map((field, index) => ({
        item: formatFieldName(field.field_name),
        specification: field.value ?? '—',
        fieldKey: `${field.field_name}#${index}`,
      })),
    }
  }, [product])
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      >
        ...
        >
          <ProductReviewPanel
            ...
            sourceCount={product?.fields.filter((f) => f.source_snippet).length}
            selectedFieldKey={selectedField?.key}
            onFieldSelect={setSelectedField}
            onInspectContradiction={setInspectedField}
      ...
      <ContradictionModal
        ...
        onClose={() => setInspectedField(null)}
      />
    </div>
  )
}
