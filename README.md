...
```
  ...
  url                │                   │  requests + BeautifulSoup
        ...
        products / product_fields    7. Batch Runner
        validation_flags             8. Export (JSON/CSV)
           (Supabase / Postgres)     9. Review Dashboard (React)
```
...
1. Implement `batch_runner.run_batch` over the same stage 1–6 path.
2. Add export (JSON/CSV) and the React review dashboard.
