...
```
  ...
  url                │                   │  requests + BeautifulSoup
        ...
        products / product_fields    6. Enrichment (2nd LLM call)
        validation_flags             7. Batch Runner
           (Supabase / Postgres)     8. Export (JSON/CSV)
...
- **Honest implementation status:** stages 1 (input handler), 2 (extraction), 3 (schema registry), 4 (confidence engine), and 5 (contradiction detection) are implemented and wired into `POST /api/ingest`. Stages 6 (enrichment), 7 (batch mode), 8 (export), and 9 (review dashboard) are stubs only — each has a typed contract and docstring in its module under `backend/pipeline/`, but calling them raises `NotImplementedError`. There is no frontend yet.
...
1. Implement `enrichment.enrich` as a second, clearly-marked LLM call.
2. Implement `batch_runner.run_batch` over the same stage 1–5 path.
3. Add export (JSON/CSV) and the React review dashboard.
